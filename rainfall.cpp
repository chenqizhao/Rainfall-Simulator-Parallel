...
std::vector<std::vector<std::string> > lowest_neighbours; //lowestNeighbors
...
void calc_flux(int row, int col) {
...
int rain(int t, int rain_time, int timestep) {
...
int main (int argc, char const *argv[]) { 
  ...
  int total_timestep = rain(t, rain_time, timestep);

  clock_gettime(CLOCK_MONOTONIC, &end_time);
  double elapsed_s = calc_time(start_time, end_time) / 1000000000.0;
  std::cout << "running time for " << N << "is "<< elapsed_s << std::endl;
